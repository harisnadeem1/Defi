  ...
  return (
    <div className="flex items-start">
      <div className="flex flex-col items-center mr-6">
        ...
        {!isLastModule && (
          <div className="w-1 bg-gradient-to-b from-slate-300 to-slate-200 h-full min-h-[100px] rounded-full" />
        )}
      </div>
      ...
      >
        ...
        {module.estimatedTime && (
            <p className="text-xs text-muted-foreground mb-3">Estimated time to complete: {module.estimatedTime}</p>
        )}
        ...
        <p className="text-xs text-muted-foreground mt-1 text-right">Video Placeholder - 15% Complete</p>
          ...
          <div className="mt-5 pt-4 border-t border-border">
            ...
            <ul className="list-none space-y-1 pl-0">
              {module.nextSteps.map((step, i) => (
                <li key={i} className="text-sm text-foreground/80 hover:text-primary transition-colors">
                  {step}
                </li>
              ))}
            </ul>
         ...
         {module.id === "impermanent_loss" && (
          <div className="mt-4 p-3 bg-destructive/10 rounded-lg border border-destructive/30">
            ...
            <p className="text-sm text-destructive/80 leading-relaxed">
              DeFi involves significant risks, including smart contract vulnerabilities and market volatility. Always Do Your Own Research (DYOR) and never invest more than you can afford to lose.
            </p>
          </div>
        )}
      </motion.div>
